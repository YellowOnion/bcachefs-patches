...
for version in "v5.11 origin/master 2020-12-20" "v5.10 origin/bcachefs-v5.10 2020-12-13" "v5.9 origin/bcachefs-v5.9 2020-07-22"; do
    set - $version; tag=$1; branch=$2; oldest=$3;
    echo "starting patches for $tag $branch till date $oldest"
    COMMIT=$(git log $tag remotes/$branch --format='%ad %H %h' --date=short -n 1)
    set olddate="no"
    ...
    while [ "$olddate" != "$date" ]; do
        ...
        COMMIT=$(git log $tag remotes/$branch --since="$oldest 00:00:00" --format='%ad %H %h' --before="$date 00:00:00" --date=short -n 1)
        olddate=$date
        set - $COMMIT; date=$1; id=$2; short=$3;
    done
done
